...
def mock_draft(category: str, masked: str):
    ...
    if category == "Password" and "reset password" in faq:
        return (
            "Reset via portal SSO → Lupa Password → verifikasi NIK+OTP → buat password baru 8+ karakter. Jika gagal, buat ticket dengan NIK.",
            "FAQ: Reset Password",
        )
    if category == "Hardware-Printer":
        return (
            "Cek kabel power+jaringan, restart printer, cek antrean print. Jika lampu merah, foto error buat ticket Hardware-Printer.",
            "FAQ: Printer Error",
        )
    if category == "Software":
        return (
            "Request install via ticketing + approval atasan, IT install remote via SCCM. Jangan install sumber tidak resmi.",
            "FAQ: Install Software",
        )
    if category == "Network":
        return (
            "Coba reconnect/lupakan jaringan. Jika semua device down, lapor IT Network dengan lokasi lantai.",
            "FAQ: Jaringan / WiFi",
        )
    if category == "Email":
        return (
            "Cek spam/junk & kuota mailbox, coba webmail. Jika tetap, kirim screenshot error.",
            "FAQ: Email Tidak Masuk",
        )
    if category == "ERP":
        return (
            "Pastikan VPN aktif + role approval. Error 403 = cek VPN/role, hubungi IT dengan NIK & modul ERP.",
            "FAQ: Akses ERP",
        )
    return (
        "Terima kasih laporannya, tim IT akan cek. Mohon sertakan lokasi & screenshot jika ada.",
        "FAQ: General",
    )
...
st.caption(
    "Auto-kategorisasi + Draft FAQ + Masking + Needs Review • Python + Streamlit + AI API • MagangHub Mustika/Bank Mega"
)
...
with st.sidebar:
    ...
    st.metric(
        ...
        "ON (Gemini)"
        if has_gem
        else ("ON (OpenAI)" if has_oai else "OFF (Mock rule-based)"),
    )
    st.info(
        "Tanpa key tetap jalan pakai mock. Isi .env: GOOGLE_API_KEY untuk Gemini gratis atau OPENAI_API_KEY."
    )
    st.divider()
...
with col1:
    ...
    ticket = st.text_area(
        "Tulis ticket (coba paste yang sensitif, akan dimask):",
        height=120,
...
if TICKETS_PATH.exists():
    ...
    st.caption(
        "Ganti mock dengan AI API untuk akurasi lebih tinggi. Log ini bisa jadi dashboard IT."
    )
else:
...
st.divider()
st.markdown(
    "**Untuk Interview:** share screen ini → tunjukkan masking + kategorisasi + Needs Review. Repo: `helpdesk-ai`"
)
